...
def register_user():
    ...
    while True:
        ...
        if len(user_id) == 4 and user_id.isdigit():
            ...
            print(
                Fore.RED +
                '  Invalid ID. It must be 4 digits long.' +
                Style.RESET_ALL)
...
def add_expense():
    ...
    try:
        Expenses.append_row(
            [current_user_id, amount, category_map[category], date])
        sleep(1.5)
        print(
            Fore.GREEN +
...
def get_report():
    ...
    try:
        ...
        rows = [row for row in records if str(
            row['user_id']) == str(current_user_id)]
        ...
        grouped = df.groupby(['month_year', 'category']).agg(
